# Iteration 1: Progress Report - Maraea Dewes

---

## Overview of Group Contributions per member

### Maraea

- regularly contributed to in-class discussions
- contributed to group discussions in group discord
- often tried to provide clarification on certain aspects of the group assignment.
- contributed to assigning tasks to group members.
- took meeting minutes for meeting one.
- worked on persona: Jennifer Michael.
- assisted on markdown related inquiries (particularly table creation)
	- helped edit markdown files to be more presentable
- did the readme.md file


### Kyle

- lead group discussions and always took part, took on group leader role.
- always asked for help when he needed it, has very good communication
- always communicated changes and progress
- always asked for feedback from the team
- worked on stakeholder register
- worked on the persona: Jaga Grahame
- took meeting notes for meeting two and partway into meeting three
- very quick in completing work up to standard

### Liam

- contributed to initial group setup:
	- group discord creation
	- github repository creation and configuration
- contributed to group meetings often
- provided help and clarification on difficult topics where needed.
- asked for help when needed

> note: often unwell so couldn't attend some days, but still communicated a lot on our group discord!

### Luke

- contributed to group meetings often
- contributed to persona ideating
- worked on persona: barry whitmore and joe rodgers
- took meeting notes for meeting three

> note: got sick for a week and eventually had to be moved into another group, was also our 4th member so we had trouble trying to allocate an equal amount of work to him. still contributed a lot!

There may be more work that teammates did but I can't remember all of it.

### Personal Reflection

- I want to try to contribute more to group discussions
- Be more timely with my own work submissions
- Be more co-operative: provide feedback on teammates work in a timely manner
	- I could do this by reviewing as soon as or near a time when a teammate has pushed their work
	- (and also asked for feedback!!)
- Try not to be too overwhelming and cut down on long-winded explanations, make my messages more concise and easier to take in for others.
- I also want to try to be more organised and try to stay on top of discussions and changes, etc.

Conclusion:
Going forward, I will dedicate more time to these assignments, be more helpful to the team and TRY to provide SHORT feedback where possible (and if wanted).

### What we could work on in the future:

- better handling of deadlines and task allocation
- better communication, organization of information (mostly my fault, I created multiple channels in our discord for different use-cases and ended up mainly using the general and git update chat)

## Conclusion:

Overall, I am a bit unhappy with my OWN personal contributions to this iteration, but I AM happy with everyone else's work! There were understandably some personal struggles among members but we as a team took that into account and still completed the work by the deadline.
